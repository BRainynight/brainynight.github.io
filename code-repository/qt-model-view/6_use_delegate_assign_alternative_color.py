...
class CheckBoxTableModel(QStandardItemModel):
    ...
    def setData(self, index, value, role=Qt.EditRole):
        if index.isValid() and role in [ Qt.CheckStateRole, Qt.UserRole]:
            if index.row() == 0:
                self.setItemData(index, {Qt.CheckStateRole:value, Qt.UserRole:value}) # Don't set to CheckStateRole!! it causes problem!!
            else:
...
class TableWidget(QWidget):
    ...
    def initUI(self):
        ...
        self.delegate = NoCheckboxDelegate(self.tableView)
